...
<body class="w-full">
    <div
        class="grid md:grid-cols-6 lg:grid-cols-5 w-full h-screen overflow-y-auto bg-black/85 bg-[url(/public/bg-img.png)] bg-cover bg-center bg-fixed  bg-blend-multiply border border-purple-400/10">
        <div
            class="col-span-6 md:col-span-3 lg:col-span-2 w-full p-0.5 h-screen border border-green-400/10 flex flex-col justify-between">
            <div class="bg-green-400/10 border border-green-400/10 p-5 h-full flex items-center">
                <form action="{{ route('signup') }}" method="POST"
                    class="flex flex-col items-center gap-5 text-white/30 w-full">
                    @csrf
                    <input type="hidden" name="ref" value="{{ request('ref') }}">
                    @if (session('error'))
                        <div
                            class="px-4 py-2 bg-red-500/20 border border-red-500 text-red-100 rounded-xl mb-4 text-sm w-full">
                            {{ session('error') }}
                        </div>
                    @endif
                    ...
                    @if (session('success'))
                        <div
                            class="px-4 py-2 bg-green-500/20 border border-green-500 text-green-100 rounded-xl mb-4 text-sm w-full">
                            {{ session('success') }}
                        </div>
                    @endif
                    <div class="flex gap-2 items-center justify-center text-xs w-fit p-1 bg-orange-400 rounded-2xl">
                        ...
                        <a href="{{ route('show.login') }}"
                            class="bg-black rounded-xl p-1 px-2 text-orange-400/80 font-medium font-mono">Log in
                            here</a>
                    </div>
                    ...
                    <div class="flex flex-col gap-2 w-full">
                        ...
                        <input id="password" type="password" name="password" autocomplete="new-password"
                            ...
                            class="p-3 wp-3 rounded-xl bg-white/5 outline outline-white/20 backdrop-blur-4xl text-orange-400/70 text-sm font-semibold placeholder-orange-400/70" />
                    ...
                    <div class="flex flex-col gap-2 w-full">
                        <label for="password_confirmation" class="text-white/60 font-medium ml-1 text-sm">Confirm
                            Password</label>
                    ...
                    <button
                        class="w-full p-3 bg-black/80 border border-green-400/10 rounded-3xl text-white/70 text-sm font-mono font-medium">Sign
                        Up</button>
        ...
        <div class="col-span-3 border border-green-400/10 hidden md:block w-full h-full relative p-0.5">
