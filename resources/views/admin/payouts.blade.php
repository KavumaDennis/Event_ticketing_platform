...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-green-400/10 border-zinc-800 overflow-hidden border">
            <div class="p-3 border-b border-zinc-800 flex justify-between items-center">
                <div>
                    <h2 class="text-xl font-bold text-white uppercase tracking-tighter">Pending Payouts</h2>
                ...
                <div class="flex items-center gap-5">
                    <div
                        ...
                        UGX {{ number_format($requests->where('status', 'pending')->sum('amount')) }}
                    </div>
                    <div class="text-xs text-zinc-600">{{ $requests->where('status', 'pending')->count() }} active
                        requests
                    </div>
                </div>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-zinc-800">
                        @forelse($requests as $req)
                            <tr class="hover:bg-zinc-800/50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    @endphp
                                    <span
                                        class="px-2 py-1 text-xs rounded border {{ $statusClasses[$req->status] ?? 'bg-zinc-800 text-zinc-400 border-zinc-700' }}">
                                        {{ ucfirst($req->status) }}
                                    </span>
                                </td>
                                    ...
                                    @if ($req->status === 'pending')
                                        <div x-data="{ open: false }" class="inline-block relative">
                                            <button @click="open = !open"
                                                class="text-orange-400 hover:text-orange-300 text-sm font-medium">Process</button>

                                            {{-- Simple Action Modal --}}
                                            <div x-show="open" x-cloak @click.away="open = false"
                                                class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
                                                <div
                                                    class="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl w-full max-w-md text-left shadow-2xl">
                                                    <h3 class="text-lg font-bold text-white mb-4">Process Payout Request
                                                    </h3>
                                                    <form action="{{ route('admin.payouts.update-status', $req->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        <div class="mb-4">
                                                            <label
                                                        ...
                                                        <div class="flex justify-end gap-3">
                                                            <button type="button" @click="open = false"
                                                                class="px-4 py-2 text-zinc-500 hover:text-white transition-colors">Cancel</button>
                                                            <button type="submit"
                                                                class="px-6 py-2 bg-orange-500 text-black font-bold rounded-xl hover:bg-orange-400 transition-colors">Confirm</button>
                                                        </div>
                                                    </form>
                                                </div>
                                            </div>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center text-zinc-500 italic">No payout requests
                                    found</td>
                            </tr>
                        @endforelse
                    </tbody>
