...
<div class="md:px-6 md:pb-20">
    ...
    <div id="content-upcoming" class="tab-content transition-all duration-500">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="overflow-hidden group hover:border-orange-500/30 transition-all duration-500 relative">
                <div class="flex flex-col sm:flex-row sm:h-40 gap-3  bg-green-400/10 border border-green-400/10 p-2 hover:border-orange-400/30 transition-all">
                    {{-- Event Graphic --}}
                    <div class="w-full sm:w-1/3 h-100 sm:h-full relative overflow-hidden">
                        <img src="{{ $ticket->event->event_image ? asset('storage/'.$ticket->event->event_image) : asset('default.png') }}" class="h-full w-full object-cover">
                    </div>
                    ...
                    <div class="flex-1 flex flex-col justify-between">
                        ...
                        <div class="flex gap-3 mt-4 sm:mt-0">
                            <a href="{{ route('dashboard.ticket.view', $ticket) }}" class="flex-1 px-3 py-1.5 bg-white/5 border border-white/20 text-orange-400 text-center rounded-lg flex items-center justify-center gap-2 hover:text-white duration-150 transition-colors text-[10px] font-bold uppercase">
                                View qr-code
                            </a>
                            <button onclick="openTransferModal({{ $ticket->id }}, '{{ $ticket->event->event_name }}')" class="flex-1 px-3 py-1.5 rounded-lg bg-orange-400 text-black text-[10px] font-bold uppercase tracking-wider hover:bg-orange-300 transition-all">
                                Gift ticket
                            </button>
                        </div>
                    </div>
    ...
    <div id="content-past" class="tab-content hidden transition-all duration-500">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="bg-green-400/10 border border-green-400/5 p-3 hover:opacity-100 transition-opacity">
                <div class="flex justify-between items-start">
                    <div>
    ...
    <div class="bg-black/85 bg-[url(/public/bg-img.png)] bg-cover bg-center bg-fixed  bg-blend-multiply border border-green-400/30 backdrop-blur-[1px] w-full max-w-lg p-4 shadow-xl relative">
        ...
        <form id="transferForm" method="POST" action="">
            ...
            <div class="mb-6">
                ...
                <input type="email" name="recipient_email" required class="w-full p-3 rounded-xl bg-white/5 outline outline-white/20 backdrop-blur-4xl text-orange-400/70 text-sm font-semibold placeholder-orange-400/70" placeholder="friend@example.com">
            </div>

            <div class="flex gap-4">
                <button type="button" onclick="closeTransferModal()" class="flex-1 px-3 py-2 rounded-lg bg-orange-400 text-black text-[10px] font-bold uppercase tracking-wider hover:bg-orange-300 transition-all">Cancel</button>
                <button type="submit" class="flex-1 px-3 py-2 rounded-lg bg-green-400 text-black text-[10px] font-bold uppercase tracking-wider  transition-all">Send Gift</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    function switchTab(tab) {
        ...
        document.querySelectorAll('.tab-btn').forEach(btn => {
            btn.classList.remove('bg-orange-400', 'text-black');
        ...
        document.getElementById('tab-' + tab).classList.add('bg-orange-400', 'text-black');
