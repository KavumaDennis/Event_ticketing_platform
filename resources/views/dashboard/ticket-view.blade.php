...
<div class="flex items-center justify-center p-4">
    <div class=" w-fit bg-green-400/10 border border-green-400/10 overflow-hidden relative">
        {{-- Decorative Header --}}
        ...
        <div class="p-8 grid grid-cols-3 items-center gap-12 relative">
            ...
            <div class="col-span-2 flex flex-col">
                ...
                <div class="text-center flex flex-col mb-8">
                    ...
                    <div class="grid grid-cols-2 gap-4 text-left bg-zinc-950/50 p-4 rounded-xl border border-zinc-800/50">
                        <div>
                            ...
                            @php
                                $transfer = $ticket->latestAcceptedTransfer;
                            @endphp
                            @if($transfer)
                                <div class="text-[10px] text-orange-400/70 font-mono mt-1 italic">Sent by {{ $transfer->sender->first_name }}</div>
                            @endif
                        </div>
                        <div>
                ...
                {{-- Actions --}}
                <div class="space-y-3">
                    <a href="{{ route('ticket.download', $ticket->ticket_code) }}" class="block w-full py-3  bg-white/5 border border-white/20 text-orange-400 rounded-lg font-medium font-mono text-center transition-colors">
                        <i class="fas fa-download mr-2"></i> Download Ticket (PDF)
                    </a>

                    <a href="{{ route('user.dashboard.tickets') }}" class="block w-full py-3 bg-orange-400 text-black border border-orange-400 font-medium font-mono text-center rounded-lg transition-colors">
                        Back to My Tickets
                    </a>
                </div>
            </div>
