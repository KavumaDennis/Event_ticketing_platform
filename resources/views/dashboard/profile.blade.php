...
<div class="flex flex-col lg:grid lg:grid-cols-4 gap-6 relative">
    <div class="flex flex-col gap-3 relative">
        ...
        <div class="col-span-1 p-4 bg-green-400/10 h-fit rounded-2xl">
            ...
            <div class="mt-3 text-white/60">
                <p class="flex items-center gap-2">
                    <span class="relative text-orange-400/70 text-sm font-medium pr-3 after:absolute after:right-0 after:w-1 after:h-3 after:bg-orange-400 after:rounded-lg flex items-center">
                        Events
                    </span>
                    <span>
                        {{ $stats['events_count'] }}
        ...
        <a href="#" id="editProfileBtn" class="flex-1 px-3 py-2.5 rounded-lg bg-orange-400 text-black text-[10px] font-bold uppercase tracking-wider hover:bg-orange-300 transition-all text-center">
            Edit profile
        </a>

        <div class="col-span-1 p-4 bg-black/40 border border-green-400/10 rounded-2xl">
            <div class="flex items-center gap-3">
                @php $hasExperience = $activeExperiences->count() > 0; @endphp
                <div class="experience-ring {{ $hasExperience ? 'experience-border' : '' }}">
                    <img src="{{ Auth::user()->profile_pic ? asset('storage/' . Auth::user()->profile_pic) : asset('default.png') }}" class="w-12 h-12 rounded-full object-cover" alt="Profile photo">
                </div>
            ...
            <form action="{{ route('experiences.store') }}" method="POST" enctype="multipart/form-data" class="mt-4 flex flex-col gap-3">
                @csrf
                <input type="file" name="media" accept="image/*,video/*" required class="w-full p-2.5 rounded-xl bg-white/5 outline outline-white/20 text-orange-400/70 text-xs font-semibold">
                <input type="text" name="caption" maxlength="255" placeholder="Say something about this experience..." class="w-full p-2.5 rounded-xl bg-white/5 outline outline-white/20 text-orange-400/70 text-xs font-semibold placeholder-orange-400/70">
                <button type="submit" class="w-full bg-orange-400 text-black/90 text-sm font-bold font-mono p-2 rounded-3xl hover:bg-orange-500 transition">
                    Share Experience
                </button>
            </form>
        </div>
        ...
        <a href="{{ route('organizer.settings') }}" class="flex-1 px-3 py-2.5 rounded-lg text-black text-[10px] font-bold uppercase tracking-wider hover:bg-orange-300 transition-all bg-green-400/70 text-center mt-2">
            Manage Organizer
        </a>
        @endif
        ...
        <div id="editProfileModal" class="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 hidden">
            <div class="bg-black/85 bg-[url(/public/bg-img.png)] bg-cover bg-center bg-fixed  bg-blend-multiply border border-green-400/30 backdrop-blur-[1px] w-full max-w-lg p-4 shadow-xl relative">
                ...
                <form action="{{ route('user.dashboard.updateProfile') }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-3">
                    ...
                    {{-- Save Button --}}
                    <button type="submit" class="mt-4 w-full bg-orange-400 text-black/90 font-mono font-medium p-2 rounded-3xl hover:bg-orange-500 transition">
                        Save Changes
                    </button>
                </form>
            </div>
    ...
    <div class="col-span-12 lg:col-span-3">
        <div class="mb-6">
            <h3 class="text-xs p-1 font-mono font-medium bg-orange-400 rounded-2xl w-fit text-black/90 mb-3 tracking-tighter">Your Experiences</h3>
                        ...
                        <div class="flex flex-col items-center gap-2 w-24">
                            <div class="experience-ring">
                                @if($exp->media_type === 'video')
                                    <video src="{{ asset('storage/' . $exp->media_path) }}" class="w-16 h-16 rounded-full object-cover" muted loop playsinline></video>
                                @else
                                    <img src="{{ asset('storage/' . $exp->media_path) }}" class="w-16 h-16 rounded-full object-cover" alt="Experience media">
                                @endif
                            </div>
                            ...
                            <form action="{{ route('experiences.destroy', $exp) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-[10px] bg-red-500/20 text-red-400 rounded-2xl px-2 py-1 hover:bg-red-500/30 transition">
                                    Delete
                                </button>
                            </form>
                        </div>
        ...
        <h3 class="text-xs p-1 font-mono font-medium bg-orange-400 rounded-2xl w-fit text-black/90 mb-3 tracking-tighter">Followed Organizers' Events</h3>
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
            ...
            <div class="w-full h-fit p-1 ">
                <div class="w-full h-[100px] rounded-2xl bg-green-400/10 relative p-2">
                    <img src="{{ $ev->event_image ? asset('storage/'.$ev->event_image) : asset('default.png') }}" onerror="this.onerror=null; this.src='{{ asset('default.png') }}';" class="absolute mix-blend-lighten z-0 top-0 left-0 object-cover w-full h-full rounded-[20px]" alt="{{ $ev->event_name }}" />
