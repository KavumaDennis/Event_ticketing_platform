...
<head>
    ...
    <style>
        ...
        @php
            ...
            $customBg = $organizer->ticket_custom_background;
            $customLogo = $organizer->ticket_custom_logo;
            
            $bgBase64 = null;
            if ($customBg && Storage::disk('public')->exists($customBg)) {
                $bgData = Storage::disk('public')->get($customBg);
                $bgBase64 = 'data:image/' . pathinfo($customBg, PATHINFO_EXTENSION) . ';base64,' . base64_encode($bgData);
            }

            $logoBase64 = null;
            if ($customLogo && Storage::disk('public')->exists($customLogo)) {
                $logoData = Storage::disk('public')->get($customLogo);
                $logoBase64 = 'data:image/' . pathinfo($customLogo, PATHINFO_EXTENSION) . ';base64,' . base64_encode($logoData);
            }
        @endphp
        body {
        ...
        .ticket-card {
            ...
            border-radius: 10px;
            background-color: {{ $bgBase64 ? 'rgba(24, 24, 27, 0.85)' : '#18181b' }}; /* semi-transparent if bg exists */
            padding: 30px;
            position: relative;
        }
        .header {
