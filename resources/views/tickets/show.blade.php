<x-layout>
    <div
        class="min-h-screen bg-black/85 flex items-center justify-center p-4 bg-[url('/public/bg-img.png')] bg-cover bg-center bg-fixed bg-blend-multiply">
        <div class="max-w-3xl w-full bg-green-400/20 border border-green-400/10 overflow-hidden relative">
            {{-- Decorative Header --}}
            <div class="relative border-b border-green-400/10">
                <div class="flex items-center justify-center">
                    <span class="text-3xl text-white/50 font-black tracking-widest py-4">TICKET</span>
                </div>
            </div>

            <div class="p-8 flex items-center gap-5 relative">
                {{-- QR Code Card --}}
                <div
                    class="bg-zinc-950 p-6 rounded-2xl border border-zinc-800 shadow-xl flex flex-col items-center">
                ...
                <div class="text-center flex-1 flex flex-col justify-between">
                    ...
                    <div
                        class="grid grid-cols-2 gap-4 text-left bg-zinc-950/50 p-4 rounded-xl mb-3 border border-zinc-800/50">
                        <div>
                            ...
                            @if ($transfer)
                                <div class="text-[10px] text-orange-400/70 mt-1 italic">Sent by
                                    {{ $transfer->sender->first_name }}</div>
                            @endif
                        </div>
                        <div>
                    ...
                    <div class="space-y-3">
                        <a href="{{ route('ticket.download', $ticket->ticket_code) }}"
                            class="block w-full py-3 uppercase text-[10px] bg-orange-400 hover:bg-green-400 text-black font-bold text-center rounded-xl transition-colors">
                            <i class="fas fa-download mr-2"></i> Download Ticket (PDF)
                        </a>

                        <a href="{{ route('home') }}"
                            class="block w-full py-3 uppercase text-[10px] bg-zinc-800 border border-white/50 hover:bg-zinc-700 text-zinc-300 font-medium text-center rounded-xl transition-colors">
                            Back to Home
                        </a>
                    </div>
                </div>
