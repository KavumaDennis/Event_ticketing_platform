<x-layout>
    ...
    <div class="p-5">
        ...
        <div x-data="{ showFollowers: false, showFollowing: false }">
                ...
                <div class="col-span-1 h-fit grid grid-cols-3 p-3 rounded-3xl border-green-400/20 bg-green-400/10">
                    <div class="col-span-2 flex flex-col gap-3 items-center justify-center">
                        ...
                        <button type="button" id="open-experiences" class="experience-ring">
                            <img src="{{ $user->profile_photo_url }}" alt=""
                                class='size-18 rounded-[50%] object-cover' />
                        </button>
                        <div class="">
                    ...
                    <div class="">
                        <div class="flex flex-col py-1">
                            ...
                            <p class="cursor-pointer text-white/60 text-sm flex items-baseline gap-1"
                            ...
                            <p class="text-white/60 text-sm flex items-baseline gap-1">
                                <span class="text-orange-400/60">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17"
                                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                        stroke-linecap="round" stroke-linejoin="round"
                                        class="lucide lucide-chart-no-axes-column-decreasing-icon lucide-chart-no-axes-column-decreasing">
                                        <path d="M5 21V3" />
                                        <path d="M12 21V9" />
                                        <path d="M19 21v-6" />
                                    </svg>
                ...
                <div class="col-span-3 flex flex-col gap-10 p-5">
                    <div class="flex flex-col gap-3">
                        <div class="flex items-center">
                            <span
                                class="pr-3 relative after:content-[''] flex items-center text-sm font-medium text-orange-400/70 after:bg-orange-400/80 after:absolute  after:w-[3px] after:h-[12px] after:rounded-lg after:right-0">
                                Phone
                            </span>
                        ...
                        <div class="flex">
                            <span
                                class="pr-3 relative after:content-[''] flex items-center h-fit  text-sm font-medium text-orange-400/70 after:bg-orange-400/80 after:absolute  after:w-[3px] after:h-[12px] after:rounded-lg after:right-0">
                                About
                            </span>
                            <span
                                class="pl-3 text-sm text-white/60 font-mono font-light">{{ $user->bio ? $user->bio : 'No bio yet' }}</span>
                        </div>

                        @if (Auth::check() && Auth::id() == $user->id)
                            <div class="mt-5 p-4 bg-green-400/10 border border-green-400/10 rounded-lg">
                                ...
                                <form action="{{ route('experiences.store') }}" method="POST"
                                    ...
                                    <div class="flex justify-between items-center gap-3">
                                        <input type="file" name="media" accept="image/*,video/*" required
                                            class="p-2 rounded-lg bg-orange-400 w-fit text-black/90 text-xs font-semibold">
                                        <input type="text" name="caption" maxlength="255"
                                            placeholder="Caption (optional)"
                                            class="w-full p-2 rounded-lg bg-[#b0a6df]/10 outline outline-[#b0a6df]/20 text-white/80 text-xs font-semibold placeholder-white/70">
                                    </div>
                                    <button type="submit"
                                        class="p-0.5 h-fit flex gap-1 items-center bg-orange-400 text-black/60 font-medium text-sm w-fit rounded-4xl">
                                        <span
                                        ...
                                        <span class="pr-2 text-black/90 font-mono text-xs font-medium">Share
                                            Experience</span>
                    ...
                    <div class="flex items-end gap-5 h-full">
                        <div>
                            @auth
                                @if (auth()->id() != $user->id)
                                    <form
                                        action="{{ $user->followers()->where('follower_id', auth()->id())->exists()? route('user.unfollow', $user): route('user.follow', $user) }}"
                                        method="POST">
                                        @csrf
                                        @if ($user->followers()->where('follower_id', auth()->id())->exists())
                                            @method('DELETE')
                                            <button
                                                class="p-0.5 rounded-3xl bg-orange-400 border border-green-400/15 flex items-center gap-1 text-sm font-semibold">
                                                <span
                                                    class="size-8 flex items-center justify-center rounded-full text-orange-400/70 bg-black/95 border border-green-400/15">
                        ...
                        <div
                            class="flex justify-between items-center gap-4 bg-orange-400 border border-green-400/10 p-0.5 rounded-3xl">
            ...
            <!-- Followers Modal -->
            <div x-show="showFollowers" x-cloak
                class="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
                <div
                    class="w-100 max-h-[400px] overflow-y-auto bg-black/85 bg-[url(/public/bg-img.png)] bg-cover bg-center bg-fixed  bg-blend-multiply relative border border-green-400/30 backdrop-blur-[1px] p-5">
                    <div class="flex w-full justify-between items-center mb-5">
                        <h2 class="text-xs p-1 font-mono font-medium bg-orange-400 rounded-2xl w-fit text-black/90">
                            Followers</h2>
                        <button @click="showFollowers = false"
                            class="w-fit p-1 bg-orange-400 text-black/90 font-medium font-mono text-sm border border-green-400/10 rounded-3xl">
                            <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15"
                                viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                        ...
                        @foreach ($followers as $f)
                            <li class="flex items-center justify-between gap-3 bg-green-400/10 p-2 rounded-2xl">
                                <div class="flex items-center gap-3">
                                    <img src="{{ $f->follower->profile_photo_url ?? asset('img2.jpg') }}"
                                        class="w-10 h-10 rounded-full" alt="{{ $f->follower->first_name }}">
                                    <a href="{{ route('user.profile', $f->follower->id) }}"
                                        class="text-white/70 hover:underline">{{ $f->follower->first_name }}
                                        {{ $f->follower->last_name }}</a>
                                </div>
                                @if (auth()->check() && auth()->id() != $f->follower->id)
                                    <form
                                        action="{{ $f->follower->followers()->where('follower_id', auth()->id())->exists()? route('user.unfollow', $f->follower): route('user.follow', $f->follower) }}"
                                        method="POST">
                                        @csrf
                                        @if ($f->follower->followers()->where('follower_id', auth()->id())->exists())
                                            @method('DELETE')
                                            <button
                                                class="bg-red-600/70 px-3 py-1.5  border border-white/20 text-orange-400 text-center rounded-lg flex items-center justify-center gap-2 hover:text-white duration-150 transition-colors text-[10px] font-bold uppercase shrink-0">Unfollow</button>
                                        @else
                                            <button
                                                class="px-3 py-1.5 bg-white/5 border border-white/20 text-orange-400 text-center rounded-lg flex items-center justify-center gap-2 hover:text-white duration-150 transition-colors text-[10px] font-bold uppercase shrink-0">Follow</button>
                                        @endif
                                    </form>
                                @endif
                            </li>
            ...
            <!-- Following Modal -->
            <div x-show="showFollowing" x-cloak
                class="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
                <div
                    class="w-100 max-h-[400px] overflow-y-auto bg-black/85 bg-[url(/public/bg-img.png)] bg-cover bg-center bg-fixed  bg-blend-multiply relative border border-green-400/30 backdrop-blur-[1px] p-5">
                    <div class="flex w-full justify-between items-center mb-5">
                    ...
                    <ul class="space-y-3">
                        @foreach ($following as $f)
                            @if ($f->following)
                                {{-- only display if followed user exists --}}
                                <li class="flex items-center justify-between gap-3 bg-green-400/10  p-2 rounded-2xl">
                                    <div class="flex items-center gap-3">
                                        <img src="{{ $f->following->profile_photo_url ?? asset('img2.jpg') }}"
                                            class="w-10 h-10 rounded-full" alt="{{ $f->following->first_name }}">
                                        <a href="{{ route('user.profile', $f->following->id) }}"
                                            class="text-white/70 hover:underline">{{ $f->following->first_name }}
                                            {{ $f->following->last_name }}</a>
                                    </div>

                                    @if (auth()->check() && auth()->id() != $f->following->id)
                                        <form
                                            action="{{ $f->following->followers()->where('follower_id', auth()->id())->exists()? route('user.unfollow', $f->following): route('user.follow', $f->following) }}"
                                            method="POST">
                                            @csrf
        ...
        <div id="experience-viewer" class="fixed inset-0 bg-black/80 hidden items-center justify-center z-50">
            <div
                class="w-full max-w-md bg-black/85 bg-[url(/public/bg-img.png)] bg-cover bg-center bg-fixed  bg-blend-multiply relative border border-green-400/30 backdrop-blur-[1px] overflow-hidden">
                <div class="flex items-center justify-between p-0">
                    <div id="experience-progress" class="flex gap-1 w-full pl-1"></div>
                    <button id="close-experiences" class="text-zinc-500 hover:text-white transition-colors px-2">
                        <i class="fa-solid fa-xmark text-lg"></i>
                    </button>
                </div>
                <div class="relative w-full h-[460px] bg-black">
                    <img id="experience-image" class="w-full h-full object-cover hidden" alt="Experience">
                    <video id="experience-video" class="w-full h-full object-cover hidden" playsinline></video>
