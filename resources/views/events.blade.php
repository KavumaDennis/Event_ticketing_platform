    ...
    <section class="p-5">
            ...
            <div class="md:col-span-6 lg:col-span-2 col-span-1 flex flex-col justify-between gap-5">
                ...
                <div class="grid grid-cols-3 rounded-2xl w-full bg-green-400/10 border border-green-400/10 gap-3 p-3">
                    ...
                    <div class="col-span-1 flex flex-col justify-center items-center gap-2 text-white/60">
                        ...
                        <div class="text-orange-400/70 text-s font-medium">
                            {{ $event->regular_price ? 'UGX ' . number_format($event->regular_price) : 'Free' }}</div>
                    </div>
                    <div class="col-span-1 flex flex-col justify-center items-center gap-2 text-white/60">
                        <div class="text-sm font-mono font-medium">Likes</div>
                        <div id="likes-count-{{ $event->id }}" class="text-orange-400/70 text-s font-medium">
                            {{ $event->likes->count() }}</div>
                    </div>
                </div>
                ...
                <div class="flex justify-between items-center">
                    <div class="flex gap-3">
                        <a href="{{ route('event.show', $event->id) }}" class='flex gap-1 items-center'>
                            <div class="flex items-center p-0.5 w-fit bg-orange-400 gap-1 rounded-3xl">
                                <span
                                    class='size-8 flex items-center justify-center rounded-[50%] bg-black/95 text-orange-400/80'>
                        ...
                        <div class="size-9 flex items-center justify-center rounded-[14px] border border-green-400/20 bg-green-400/10 save-btn"
                            data-event="{{ $event->id }}">
        ...
        {{ Auth::check() && $event->isSavedBy(Auth::user()) ? 'text-red-500' : 'text-orange-400/80' }}">
                    ...
                    <div class="flex gap-2">
                        <span
                            class="relative text-white/70 flex items-center pr-3 after:absolute after:w-1 after:rounded-lg after:h-4 after:bg-orange-400 after:right-0">
                            <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 24 24"
                                fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                stroke-linejoin="round"
                    ...
                    <script>
                        document.addEventListener('DOMContentLoaded', function() {
                            ...
                            function startCountdown() {
                                ...
                                @if (isset($event) && $event && $event->event_date)
                                    @php
                                        $targetDateTime = \Carbon\Carbon::parse($event->event_date->format('Y-m-d') . ' ' . ($event->start_time ?? '00:00:00'));
                                        $targetTimestamp = $targetDateTime->timestamp * 1000;
                                    @endphp
                    ...
                    <div class="p-3 rounded-4xl border-green-400/20 bg-green-400/10 backdrop-blur-[1px]">
                        <div class="flex justify-between items-center">
                            <div class="flex items-center gap-3">
                                <!-- Organizer Image -->
                                <div
                                    class="w-fit h-fit rounded-[50%] bg-orange-400 border border-green-400/10 p-0.5">
                                    <img src="{{ $event->organizer->organizer_image ? asset('storage/' . $event->organizer->organizer_image) : asset('default.png') }}"
                        ...
                        <a href="{{ route('organizer.details', $event->organizer->id) }}">
                            <div
                                class="w-full p-3 rounded-3xl text-white/70 text-sm font-mono bg-black/90 border border-green-400/10 text-center mt-5 font-medium hover:bg-black/95 transition cursor-pointer">
                                Organizer details
                            </div>
                        </a>
                    </div>
            ...
            <div class="md:col-span-6 lg:col-span-4 col-span-1 flex flex-col justify-between gap-3 rounded-4xl">
                <div class="flex-1 relative bg-center bg-contain">
                    <img src="{{ $event->event_image ? asset('storage/' . $event->event_image) : asset('default.png') }}"
                        onerror="this.onerror=null; this.src='{{ asset('default.png') }}';"
                        class="absolute mix-blend-lighten z-0 top-0 left-0 object-cover hidden lg:block w-full h-full rounded-4xl opacity-60"
                        alt="{{ $event->event_name }}" />
                </div>
                <div class="">
                    <p class="text-xs mx-2 p-1 font-mono font-medium bg-orange-400 rounded-2xl w-fit text-black/90">
                        Trends tagging the event</p>
                    <div class="row-span-1 flex justify-between gap-5 py-3 pb-0">
                        @forelse($event->trends as $trend)
                            <div class="flex flex-col justify-between gap-1 bg-green-400/10 p-3 rounded-3xl w-1/3">
                                ...
                                <p class='text-orange-400/60 font-mono font-light text-sm'>
                                    {{ $trend->user->first_name ?? 'Unknown' }} {{ $trend->user->last_name ?? '' }}
                                </p>
                                <p class='text-white/60 text-sm line-clamp-3'>{{ Str::limit($trend->body, 100) }}</p>
                                <a href="{{ route('trends.show', $trend->id) }}"
                                    class="bg-green-400/10 border border-green-400/20 text-xs rounded-2xl p-0.5 px-1 text-orange-400/50 font-mono w-fit mt-1">View
                                    Trend</a>
                            </div>
                        @empty
            ...
            <div class="flex gap-5 items-center">
                <form id="search-form" method="GET" action="{{ route('events') }}" class="relative">
                    <div class="h-full relative flex items-center ">
                        <input type="search" name="search" value="{{ request('search') }}"
                            class="p-2  bg-orange-400 border border-green-400/15 w-80 rounded-3xl pr-10 outline-0 text-black/80 font-medium text-sm placeholder:text-black/80"
                            placeholder="Search events..." />
                        <button type="submit"
                            class="absolute right-1 size-8 flex items-center justify-center bg-black/95 text-orange-400/80 p-2 rounded-[50%] border border-green-400/15 hover:bg-black transition">
                ...
                <div x-data="{ open: false }" class="relative">
                    ...
                    <div x-show="open" x-transition:enter="transition ease-out duration-200"
                        ...
                        class="absolute top-full right-0 mt-2 w-64 bg-[url(/public/bg-img.png)] bg-blend-darken bg-black/90 border border-purple-400/20 rounded-3xl shadow-lg p-4 z-50 backdrop-blur-[1px]">
                        <div class="flex flex-col gap-3">
                            <h3
                                class="text-orange-400/80 font-semibold text-sm mb-1 border-b border-purple-400/10 pb-2">
                                Filter by Category</h3>

                            <div class="flex flex-wrap gap-2">
                                <!-- "All" button -->
                                <a href="{{ route('home') }}"
                                    class="category-filter px-2 py-1 {{ !request('category') ? 'bg-orange-400 text-black/90' : 'bg-black/60 text-white/70 hover:bg-orange-400/50' }} border border-green-400/15 rounded-3xl text-sm font-medium transition"
                                    data-category="">
                                    All
                                </a>

                                @foreach ($categories as $category)
                                    <a href="{{ route('home', ['category' => $category, 'search' => request('search')]) }}"
                                        class="category-filter px-3 py-1 {{ request('category') === $category ? 'bg-orange-400 text-black/90' : 'bg-black/60 text-white/70 hover:bg-orange-400/50' }} border border-green-400/15 rounded-3xl text-sm font-medium transition"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            ...
            <div class="p-3 col-span-1 w-full h-fit rounded-3xl border-green-400/20 bg-green-400/10">
                <div class="flex flex-col gap-6">
                    <div class="flex justify-between items-center">
                        <h1
                            class="bg-green-400/10 border border-green-400/20 text-xs rounded-2xl p-0.5 px-1 text-orange-400/50 font-mono">
                            Tomorrow</h1>
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('.category-filter').each(function() {
                            ...
                            $(this).toggleClass('bg-orange-400 text-black/90', isMatch);
                            $(this).toggleClass(
                                'bg-black/60 text-white/70 hover:bg-orange-400/50', !isMatch
                            );
                        });
