<x-layout>
    <div class="p-5 h-fit">
        ...
        <div class="grid grid-cols-10 gap-5 mt-5">
            ...
            <div class="col-span-10 lg:col-span-9 grid grid-cols-3 gap-5 p-2 rounded-4xl h-fit bg-green-400/10">
                ...
                <div class="h-full p-2 col-span-3 md:col-span-1 flex flex-col gap-3 rounded-3xl bg-green-400/10">
                    <div class="w-full h-[180px] md:h-[150px] lg:h-[180px]">
                        <img src="{{ $trend->image ? asset('storage/'.$trend->image) : asset('default.jpg') }}" class="w-full h-full rounded-2xl object-cover" alt="{{ $trend->title }}" />
                    </div>
                    <div class="flex flex-col justify-between h-full">
                        <p class="text-white/70 md:text-xs lg:text-sm font-light font-mono p-1 md:mb-3 lg:mb-5">{{ Str::limit($trend->body, 200) }}</p>
                        <div class="flex justify-between items-center p-0.5">
                            ...
                            <div class="flex items-center gap-1">
                                <button class="trend-like-btn cursor-pointer size-9 flex items-center justify-center border border-green-400/20 bg-green-400/10 rounded-[14px] font-medium" data-trend-id="{{ $trend->id }}" data-is-liked="{{ isset($trend->is_liked) && $trend->is_liked ? 'true' : 'false' }}">
        ...
        <div class="grid grid-cols-10 gap-5">
            <div class="col-span-10 md:col-span-6 lg:col-span-7">
                <div class="relative w-full h-fit p-5 rounded-4xl bg-black/70 bg-blend-darken overflow-hidden">
                    ...
                    <h1 class="text-4xl md:w-full lg:w-[70%] text-orange-400/70">Discover, experience and at the same time get the chance to create your own events</h1>
                    ...
                    <div class="grid grid-cols-4 gap-2 md:flex justify-between items-center">
                        ...
                        <div class="flex flex-col justify-center items-center">
                            <div class="border border-black/70 rounded-[50%] p-1 bg-orange-400/60 w-fit">
                                <img src="{{ $creator->profile_pic ? asset('storage/'.$creator->profile_pic) : asset('user-default.jpg') }}" class='size-12 md:size-10 lg:size-18 rounded-[50%]' alt="{{ $creator->first_name }}" />
                            </div>
            ...
            <div class="col-span-10 md:col-span-4 lg:col-span-3 flex flex-col gap-2 justify-between w-full h-full p-3 bg-green-400/10 rounded-4xl">
                ...
                <div class="p-1 border border-green-400/20 bg-green-400/10 rounded-3xl">
                    <ul class='flex justify-between items-center'>
                        <li class='p-1 px-3 flex items-center gap-2 rounded-3xl text-xs font-mono font-medium text-black/70 bg-pink-400/70'>
                            <span>
                        ...
                        <li class='p-1 px-3 flex items-center gap-2 rounded-3xl text-xs font-mono font-medium text-black/70 bg-blue-400/70'>
                ...
                <div class="w-auto h-fit overflow-hidden">
                    <img src="{{ $topEvent->image ? asset('storage/'.$topEvent->image) : asset('default.jpg') }}" class='w-full h-60 object-cover rounded-3xl' alt="{{ $topEvent->title }}" />
                </div>
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-1">
                        <button class="like-btn cursor-pointer size-8 flex items-center justify-center border border-green-400/20 bg-green-400/10 rounded-[14px] font-medium" data-event="{{ $topEvent->id }}">
         ...
         {{ Auth::check() && $topEvent->isLikedBy(Auth::user()) ? 'text-red-500' : 'text-orange-400/80' }}">
                        ...
                        <!-- Like count -->
                        <span id="likes-count-{{ $topEvent->id }}" class="text-white/60 text-sm font-medium after:ml-0.5 relative after:w-1 after:bg-white/60 after:rounded-xl after:h-3 flex items-center">
    ...
    <div class="p-5">
        <div class="p-5 flex justify-between items-center w-full bg-green-400/10 rounded-3xl">
            <h1 class='w-[50%] text-white/70 font-light text-xs lg:text-base font-mono'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Saepe tenetur animi amet aut, sed laboriosam.</h1>
