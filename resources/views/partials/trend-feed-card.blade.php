...
@foreach ($trends as $trend)
    <div class="trend-card mb-8 bg-green-400/10 border border-green-400/5 transition-all hover:border-orange-400/20 shadow-xl"
        data-id="{{ $trend->id }}">
            ...
            <div class="flex items-center gap-3">
                <div class="relative">
                    <div class="size-10 rounded-full p-[2px] bg-gradient-to-tr from-orange-500 to-yellow-300">
                        <x-user-avatar :user="$trend->user"
                            size="size-full rounded-full object-cover border-2 border-black" />
                    </div>
                </div>
                <div>
                    <h3 class="text-white font-bold text-sm tracking-tight capitalize">{{ $trend->user->first_name }}
                        {{ $trend->user->last_name }}</h3>
        ...
        <div class="relative overflow-hidden {{ $aspectClass }} bg-black/50 group/media" x-data="{
            currentIndex: 0,
            ...
            <div class="flex transition-transform duration-500 ease-out h-full"
                ...
                @forelse($trend->media as $media)
                    <div class="w-full h-full flex-shrink-0 relative">
                        @if ($media->type === 'video')
                            <video src="{{ asset('storage/' . $media->path) }}" class="w-full h-full object-cover"
                                autoplay muted loop playsinline :x-ref="'videoPlayer' + {{ $loop->index }}"></video>
                        @else
                ...
                @empty
                    ...
                    <div class="w-full h-full flex-shrink-0">
                        @if ($trend->is_video)
                            <video src="{{ $trend->first_media_url }}" class="w-full h-full object-cover" autoplay
                                muted loop playsinline></video>
                        @else
                            <img src="{{ $trend->first_media_url }}" class="w-full h-full object-cover"
                                alt="{{ $trend->title }}">
                        @endif
                    </div>
                @endforelse
        ...
        "
            ...
            @if ($trend->event_id)
                <div class="absolute bottom-4 left-4 z-10">
                    <a href="{{ route('event.show', $trend->event_id) }}"
                        class="flex items-center gap-2 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10 text-xs text-white/90 hover:bg-orange-400 hover:text-black transition-all">
        ...
        <div class="p-4">
            <div class="flex items-center gap-3 mb-3">
                <div class="flex items-center gap-1.5">
                    <button onclick="toggleLike({{ $trend->id }}, this)"
                        class="interaction-btn group/like {{ $trend->is_liked ? 'text-orange-400' : 'text-white/80' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 24 24"
                            fill="{{ $trend->is_liked ? 'oklch(75% 0.183 55.934)' : 'none' }}" stroke="currentColor"
                ...
                <button onclick="toggleComments({{ $trend->id }})"
                    class="interaction-btn group/comment text-white/80">
                ...
                <button
                    onclick="openShareModal({{ $trend->id }}, '{{ addslashes($trend->title) }}', '{{ route('trends.show', $trend->id) }}')"
                    class="interaction-btn group/share text-white/80">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                        fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
            ...
            <div class="space-y-2">
                ...
                <div x-data="{ expanded: false }" class="text-sm">
                    <span
                        class="font-semibold text-orange-400/90 mr-1 capitalize">{{ $trend->user->first_name }}:</span>
                    <span class="text-white/60 font-light leading-relaxed"
                        x-show="!expanded">{{ Str::limit($trend->body, 120) }}</span>
                    <span class="text-white/60 font-light leading-relaxed" x-show="expanded"
                        x-cloak>{{ $trend->body }}</span>

                    @if (strlen($trend->body) > 120)
                        <button @click="expanded = !expanded"
                            class="text-orange-400/80 hover:text-orange-400 text-xs font-bold ml-1 transition-colors"
                            x-text="expanded ? 'show less' : '...more'"></button>
                    @endif
                </div>

                {{-- Comment Section --}}
                <div id="comments-section-{{ $trend->id }}"
                    class="mt-4 pt-4 border-t border-white/5 space-y-3 hidden">
                    <div id="comments-list-{{ $trend->id }}"
                        class="space-y-3 max-h-40 overflow-y-auto pr-2 custom-scrollbar">
                        @foreach ($trend->comments->take(5) as $comment)
                            <div class="flex gap-2 text-xs">
                                <span
                                    class="font-bold text-white shrink-0 capitalize">{{ $comment->user->first_name }}:</span>
                    ...
                    <div class="flex gap-2 mt-3 pt-3 border-t border-white/5">
                        <input type="text" id="comment-input-{{ $trend->id }}" placeholder="Add a comment..."
                            class="flex-1 bg-transparent text-xs text-white outline-none placeholder-zinc-600">
                        <button onclick="submitComment({{ $trend->id }})"
                            class="text-orange-400 text-xs font-bold hover:text-orange-300 transition-colors uppercase tracking-tight">Post</button>
                    </div>
                </div>

                @if ($trend->comments_count > 0)
                    <button id="view-comments-btn-{{ $trend->id }}" onclick="toggleComments({{ $trend->id }})"
                        class="text-zinc-500 text-[10px] uppercase font-bold tracking-widest hover:text-orange-400/80 transition-colors pt-2 block">
