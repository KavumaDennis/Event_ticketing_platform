<x-layout>
    ...
    <section class="p-5">
        <div class="flex flex-col gap-4 items-center">
            <div class="flex md:flex-row flex-col items-center gap-4">
                <a href="" class="bg-orange-400 border border-green-400/15 text-black/80 font-medium p-2 px-3 rounded-3xl font-mono text-sm">Contact Us</a>
            ...
            <div class="flex items-center gap-2 bg-orange-400 p-0.5 rounded-3xl font-semibold border border-green-400/15">
    ...
    <section class="p-5">
        <div class="flex flex-col md:flex-row items-center gap-3 md:items-end justify-between mb-5">
            ...
            <div class="flex justify-between items-center md:gap-4 gap-6 bg-orange-400 border border-green-400/10 p-0.5 rounded-3xl">
    ...
    <section class="p-5">
                ...
                <div class="flex flex-col justify-between col-span-2 pr-3">
                    ...
                    <div class="flex items-end justify-between w-full">
                        <div class="text-white/70 flex gap-3 items-center">
                            <button id="openReviewsModal" class="flex items-center font-mono text-xs font-medium text-black/90 gap-1 bg-orange-400 p-0.5 rounded-3xl">
                        ...
                        <div class="flex gap-3 bg-black/90 p-0.5 border border-green-400/10  rounded-3xl">
                            <!-- Left Arrow -->
                            <span @click="prevReview()" class='size-8 flex justify-center items-center text-md bg-orange-400 text-black p-2 rounded-[50%] cursor-pointer hover:bg-orange-500 transition'>
                            ...
                            <!-- Right Arrow -->
                            <span @click="nextReview()" class='size-8 flex justify-center items-center text-md bg-orange-400 text-black p-2 rounded-[50%] cursor-pointer hover:bg-orange-500 transition'>
                                <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div id="reviewsModal" class="fixed inset-0 bg-black/30 hidden items-center justify-center z-50">
            <div class="bg-black/85 bg-[url(/public/bg-img.png)] bg-cover bg-center bg-fixed  bg-blend-multiply relative border border-green-400/30 backdrop-blur-[1px] w-full max-w-2xl p-6">
                ...
                <form id="reviewForm" class="flex flex-col gap-2 mb-4">
                    ...
                    <textarea name="body" placeholder="Write your review..." class="flex-1 p-3 rounded-xl bg-white/5 outline outline-white/20 backdrop-blur-4xl text-orange-400/70 text-sm font-semibold placeholder-orange-400/70" maxlength="500" rows="4"></textarea>

                    <button type="submit" class="w-full p-3 bg-black/80 border border-green-400/20 rounded-3xl text-white/80 text-sm font-semibold hover:bg-green-400/10 transition-all mt-4">Post</button>
                </form>
                @endauth
        ...
        <div class="flex flex-col md:flex-row justify-between md:items-center gap-3 mb-6">
            ...
            <div class="flex gap-5 items-center">
                ...
                <div x-data="{ open: false }" class="relative">
                    <button @click="open = !open" class="w-fit bg-orange-400 border border-green-400/15 p-0.5 rounded-3xl flex items-center gap-1 cursor-pointer hover:bg-orange-400/80 transition">
                    ...
                    <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 transform scale-100" x-transition:leave-end="opacity-0 transform scale-95" @click.away="open = false" class="absolute top-full right-0 mt-2 w-64 bg-[url(/public/bg-img.png)] bg-blend-darken bg-black/90 border border-purple-400/20 rounded-3xl shadow-lg p-4 z-50 backdrop-blur-[1px]">
                        <div class="flex flex-col gap-3">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <!-- "All" button -->
                                <a href="{{ route('home') }}" class="category-filter px-2 py-1 {{ !request('category') ? 'bg-orange-400 text-black/90' : 'bg-black/60 text-white/70 hover:bg-orange-400/50' }} border border-green-400/15 rounded-3xl text-sm font-medium transition" data-category="">
                                    All
                                </a>

                                @foreach($categories as $category)
                                <a href="{{ route('home', ['category' => $category, 'search' => request('search')]) }}" class="category-filter px-3 py-1 {{ request('category') === $category ? 'bg-orange-400 text-black/90' : 'bg-black/60 text-white/70 hover:bg-orange-400/50' }} border border-green-400/15 rounded-3xl text-sm font-medium transition" data-category="{{ $category }}">
                                    {{ ucfirst($category) }}
                                </a>
                                @endforeach
                            </div>
