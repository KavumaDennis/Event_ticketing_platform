<x-layout>
    <section class="p-5">
        <div class="grid  lg:grid-cols-2 gap-10 p-5 bg-green-400/10 border border-green-400/5 rounded-3xl">
            <div class="col-span-1 flex flex-col gap-3 justify-center">
                ...
                <p class="text-white/70 font-mono font-light text-sm">Whether its your first events, or your biggest
                    event, we are here to help you make it happen</p>
            </div>
            <div class="col-span-1 flex justify-between items-center gap-3">
                <div class="p-5 px-5  flex flex-col items-center justify-center gap-3 bg-green-400/10 border border-green-400/5 rounded-2xl">
                    <p class="text-white/60 font-medium">Automated Emails</p>
                    <div class="text-orange-400/70">
                        <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                            stroke-linejoin="round" class="lucide lucide-mails-icon lucide-mails">
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
            <div class="w-full h-60 rounded-3xl flex flex-col justify-between pt-5">
                <div class="flex flex-col gap-5">
                    ...
                    <h1 class="text-2xl text-orange-400/70">Get your eventing journey started, create your first event
                    </h1>
                </div>
                <a href="{{ route('organizer.signup') }}">
                    <div
                        class='p-1 flex gap-1 items-center bg-orange-400 text-black/60 font-medium text-sm w-fit rounded-4xl'>
            ...
            <div class="flex items-center rounded-2xl bg-green-400/10 border border-green-400/5 overflow-hidden col-span-2">
                <!-- Pro Boost Card -->
                <div
                    class="w-full h-60 flex flex-col justify-between  p-4 relative overflow-hidden group">
                    <div class="relative z-10">
                        <div class="flex items-center gap-2 mb-2">
                            <span
                                class="bg-orange-400/20 text-orange-400 text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider">Most
                                Popular</span>
                        </div>
                        <h3 class="text-orange-400 font-bold text-xl mb-1">Pro Boost</h3>
                        <p class="text-white/60 text-xs leading-relaxed font-mono">Analytics + 5x Boosted Reach. Make
                            your
                            events stand out.</p>
                    </div>
                    <div class="relative z-10">
                        ...
                        <form action="{{ route('organizer.tier.buy') }}" method="POST">
                            ...
                            <button type="submit"
                                class="w-full bg-orange-400 hover:bg-orange-400 text-black tracking-tighter text-sm uppercase font-medium py-2 rounded-lg transition-all shadow-lg shadow-orange-400/10">Upgrade
                                to Pro</button>
                        </form>
                    </div>
                    <div
                        class="absolute -right-10 -top-10 w-32 h-32 bg-orange-400/5 rounded-full blur-3xl group-hover:bg-orange-400/15 transition-all">
                    </div>
                </div>
                ...
                <div
                    ...
                    <div class="relative z-10">
                        <div class="flex items-center gap-2 mb-2">
                            <span
                                class="bg-green-400/20 text-green-400 text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider">Max
                                Visibility</span>
                        </div>
                        <h3 class="text-green-400 font-bold text-xl mb-1">Elite Boost</h3>
                        <p class="text-white/60 text-xs leading-relaxed font-mono">Homepage & Trends Priority. Be the
                            first
                            thing users see.</p>
                    </div>
                    <div class="relative z-10">
                        ...
                        <form action="{{ route('organizer.tier.buy') }}" method="POST">
                            ...
                            <button type="submit"
                                class="w-full bg-green-400/60 hover:bg-green-400 text-black tracking-tighter text-sm uppercase font-medium py-2 rounded-lg transition-all shadow-lg shadow-green-400/10">Go
                                Elite</button>
                        </form>
                    </div>
                    <div
                        class="absolute -right-10 -top-10 w-32 h-32 bg-green-400/5 rounded-full blur-3xl group-hover:bg-green-400/15 transition-all">
                    </div>
                </div>
            </div>
            <div class="w-full h-60 rounded-2xl bg-green-400/10 border border-green-400/5 flex flex-col justify-between p-5">
                <h1 class="text-orange-400/70 text-2xl">Hey organizer, Akavaako offers a better experience when you have
                    an account </h1>
                <div
                    class="flex justify-between items-center gap-3 p-1 bg-orange-400 border border-green-400/10 rounded-3xl pl-3">
                    <a href="{{ route('events.create') }}" class='text-black/80 font-medium font-mono text-xs'>Create
                        an event</a>
                    @guest
                        <a href="{{ route('show.login') }}"
                            class='bg-black/90 border border-green-400/15 text-xs font-medium text-orange-400/60 p-2 px-3 rounded-3xl flex items-center'>Log
                            In</a>
                    @endguest

                    @auth
                        <a href="{{ route('user.dashboard.events') }}"
                            class='bg-black/90 border border-green-400/15 text-xs font-medium text-orange-400/60 p-2 px-3 rounded-3xl flex items-center font-mono text-xs'>Manage
                            events</a>
                    @endauth
                </div>
            </div>
    ...
    <section class="p-5">
        <div class="flex items-center justify-between mb-5">
            <h1 class="text-2xl text-white/90">Discover fellow organizers</h1>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
            @foreach ($organizers as $organizer)
                ...
                <div class="flex justify-between mb-5">
                    ...
                    <div class="text-sm w-fit bg-white/5 border border-white/20 text-orange-400 flex items-center rounded-md h-fit px-1.5">
                        <span class='pr-2.5 text-sm font-medium flex items-center relative after:content-[""] after:bg-orange-400 after:absolute after:w-[3px] after:h-3 after:rounded-lg after:right-0'>
                            Events
                        </span>
                ...
                <div class="flex justify-between items-center pt-5 border-t border-white/30">
                    ...
                    <a href="{{ route('organizer.details', $organizer->id) }}" class='p-1 flex gap-1 items-center bg-black/70 border border-green-400/15 text-sm w-fit rounded-4xl'>
                        <span>
                            <p class='size-7.5 flex items-center justify-center rounded-[50%] text-black/90 bg-orange-400 border border-green-400/10 text-md'>
